...
class feed {
		...
		$output = '<div class="playlist-actions-container">
						<div class="playlist-actions">
							...
							<div class="playlist-stats">
								<div class="tracks-small-icon float-right">'.$tracks[0].' <span class="playlist-stats-text">'.$LNG['tracks'].'</span></div>
							</div>
						</div>
					</div>';
		return $output;
		...
		if(empty($error)) {
			...
			if($type) {
				if(isset($_FILES['track']['name'])) {
					...
					if((!empty($ext) && !$track['valid']) || empty($ext)) {
	...
	function getActions($id, $type = null, $url = null) {
		...
		if(!empty($get['download'])) {
            if(empty($this->id)) {
                $download = '<div class="download-button" onclick="connect_modal()" title="'.$LNG['download'].'"></div>';
            } else {
...
class player {
	...
	function getEmbed($id) {
		...
		if($query->num_rows) {
			...
			return '<div class="embed-error-container" style="background: url('.$this->url.'/uploads/media/default.png) top center no-repeat #b6b6b6;" class="embed-error"><div class="embed-error-title"><a href="'.$this->url.'" target="_blank">'.$this->title.'</a></div><div class="embed-error-desc"><a href="'.$this->url.'" target="_blank">'.$LNG['embed_error'].'</a></div></div>';
		}
		return $track;
	}
	...
	function embedStats($id) {
		...
		// Output variable
		$actions = '<div class="track-likes '.((empty($people)) ? 'm-0' : '').'" id="users_likes'.$id.'">'.$people.'<a href="'.permalink($this->url.'/index.php?a=track&id='.$id).'" target="_blank"><div class="track-stats">'.$views_stats.$comments_stats.$likes_stats.$downloads_stats.'</div></a></div>';
