...
function PageMain() {
	...
	if(is_array($user) && $user['username']) {
		...
		if(isset($_GET['b']) && $_GET['b'] == 'security') {
			...
			$TMPL['image'] = '<img src="'.permalink($CONF['url'].'/image.php?t=a&w=100&h=100&src='.$user['image']).'" width="84" height="84" />';
			$TMPL['cover'] = '<img src="'.permalink($CONF['url'].'/image.php?t=c&w=900&h=200&src='.$user['cover']).'" />';
